...
class ConvBNReLU(nn.Module):
    def __init__(self, C_in, C_out, kernel_size=3, stride=1, groups=1, bias=True,dilation=1, norm_layer=nn.BatchNorm2d, padding=None):
        super().__init__()
...
class PositionalEmbedding(nn.Module):
  def __init__(self, d_model, max_len=512):
    super().__init__() # same as in deciwatch
    ...
    pe = pe.unsqueeze(0)
    self.register_buffer('pe', pe)  #(1, max_len, D), not a learnable parameter
