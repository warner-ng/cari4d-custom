...
# shared
PROJECT_ROOT="/home/warner/_projects/cari4d_clean"
...
OPT_VIZ_BATCH="${OPT_VIZ_BATCH:-8}" # step 4.8 visualization render batch; does not change optimization batch
OPT_TEMP_WEIGHT="${OPT_TEMP_WEIGHT:-1500}" # step 4.8 temporal smoothness; larger means less jitter
...
if [[ -f "${PIPELINE_OPT_OUT}" ]]; then
  ...
  run_pipeline_cmd env PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CARI4D_OPT_VIZ_BATCH="${OPT_VIZ_BATCH}" python learning/training/opt_refineout.py num_steps=3000 w_acc_v=600 w_contact=300 save_name=optv2 batch_size=64 opt_rot=True \
    opt_trans=True w_temp="${OPT_TEMP_WEIGHT}" w_sil=0.002 w_contact=200.0 w_pen=2.0 w_j2d=0.006 opt_smpl_trans=False opt_betas=False \
