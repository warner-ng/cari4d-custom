...
def geodesic_distance(R1, R2):
    ...
    The distance is the angle of the relative rotation.

    Args:
        R1 (torch.Tensor): A batch of rotation matrices of shape (B, 3, 3).
        R2 (torch.Tensor): A batch of rotation matrices of shape (B, 3, 3).

    Returns:
        torch.Tensor: A tensor of shape (B,) containing the geodesic distances in radians.
    """
    assert R1.shape == R2.shape and R1.shape[-2:] == (3, 3), f'invalid input shapes: {R1.shape}, {R2.shape}'

    # Calculate the relative rotation R_rel = R1 * R2^T
    R_rel = torch.matmul(R1, R2.transpose(-1, -2))

    # Compute the trace of each rotation matrix in the batch
    # The trace is a + e + i for a 3x3 matrix [[a,b,c],[d,e,f],[g,h,i]]
    trace = torch.diagonal(R_rel, dim1=-2, dim2=-1).sum(-1)

    # The angle of rotation is acos((trace - 1) / 2)
    # Clamp the argument to acos to the valid range [-1, 1] for numerical stability
    cos_theta = (trace - 1) / 2.
